...
func NewHTTPStaticServer(root string) *HTTPStaticServer {
	...
	m.HandleFunc("/unzip", s.hUnZip).Methods("POST")
	return s
}
...
func (s *HTTPStaticServer) hUnZip(w http.ResponseWriter, r *http.Request) {
	...
	lastExt:=strings.LastIndex(relPath,".")
	unzipPath:=relPath[:lastExt]
	if Exists(unzipPath){
		http.Error(w,"解压的目录已经存在",500)
		return
	}
	err=unzipFile(relPath,unzipPath)
	if err!=nil{
		http.Error(w,err.Error(),500)
		return
	}
